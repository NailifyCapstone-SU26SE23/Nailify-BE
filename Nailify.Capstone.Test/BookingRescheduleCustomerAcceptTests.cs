...
{
    public class BookingRescheduleCustomerAcceptTests
    {
        ...
        private readonly Guid _otherCustomerId = Guid.NewGuid();
        ...
        public BookingRescheduleCustomerAcceptTests()
        {
        ...
        private Booking CreateBooking(
            BookingStatus status = BookingStatus.RescheduleSuggested,
            string proposedBy = "Manager",
            ...
            bool hasProposedTime = true)
        {
            return new Booking
            {
                ...
                CustomerId = customerIdOverride ?? _customerId,
                SalonId = _salonId,
        ...
        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID01_ValidRequest_ShouldReturnSuccess()
        {
            // Arrange
            ...
            _notificationServiceMock.Setup(n => n.SendNotificationToSalonStaffAsync(_salonId.ToString(), "BookingRescheduleAccepted", It.IsAny<object>()))
            ...
            // Act
            var result = await _service.CustomerAcceptSuggestedTimeAsync(_bookingId, _customerId);

            // Assert
            result.IsSucceeded.Should().BeTrue();
            result.Message.Should().Be("Chấp nhận đề xuất giờ hẹn mới thành công.");
            ...
            _notificationServiceMock.Verify(n => n.SendNotificationToSalonStaffAsync(_salonId.ToString(), "BookingRescheduleAccepted", It.IsAny<object>()), Times.Once);
        }

        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID02_NotOwner_ShouldReturnError()
        {
            // Arrange
            var booking = CreateBooking();
            _bookingRepoMock.Setup(r => r.GetBookingDetailAsync(_bookingId, true))
            ...
            // Act
            var result = await _service.CustomerAcceptSuggestedTimeAsync(_bookingId, _otherCustomerId);
        ...
        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID03_InvalidStatus_ShouldReturnError()
        {
            // Arrange
            ...
            // Assert
            result.IsSucceeded.Should().BeFalse();
            result.Message.Should().Be("Không có đề xuất giờ hẹn nào từ phía Salon.");
            _unitOfWorkMock.Verify(u => u.SaveChangesAsync(), Times.Never);
        }

        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID04_ProposedByCustomer_ShouldReturnError()
        {
            // Arrange
            var booking = CreateBooking(proposedBy: "Customer");
            _bookingRepoMock.Setup(r => r.GetBookingDetailAsync(_bookingId, true))
                .ReturnsAsync(booking);
        ...
        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID05_MissingProposedTime_ShouldReturnError()
        {
            // Arrange
        ...
        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID06_ArtistCapacityConflict_ShouldReturnError()
        {
            // Arrange
            var booking = CreateBooking();
            _bookingRepoMock.Setup(r => r.GetBookingDetailAsync(_bookingId, true))
        ...
        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID07_SaveChangesAsyncThrowsException_ShouldThrowDbUpdateException()
        {
            // Arrange
            var booking = CreateBooking();
            _bookingRepoMock.Setup(r => r.GetBookingDetailAsync(_bookingId, true))
            ...
            // Act
            Func<Task> act = async () => await _service.CustomerAcceptSuggestedTimeAsync(_bookingId, _customerId);
        ...
        [Fact]
        public async Task CustomerAcceptSuggestedTimeAsync_UTCID08_NotificationServiceThrowsException_ShouldStillReturnSuccess()
        {
            // Arrange
            var booking = CreateBooking();
            _bookingRepoMock.Setup(r => r.GetBookingDetailAsync(_bookingId, true))
